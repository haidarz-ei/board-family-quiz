...
const isPreloadedSound = (sound: Howl | null) => {
  if (!sound) return false;
  return sound === regularAnswerSound || sound === highestAnswerSound || sound === wrongAnswerSound;
};

export const useRevealSound = (sfxVolume: number = 0.7) => {
  ...
  const currentStrikeSoundRef = useRef<Howl | null>(null);
  const revealTimeoutRef = useRef<ReturnType<typeof setTimeout> | null>(null);

  useEffect(() => {
    regularAnswerSound.volume(sfxVolume);
    highestAnswerSound.volume(sfxVolume);
    wrongAnswerSound.volume(sfxVolume);
  }, [sfxVolume]);
  ...
  const playRevealSound = useCallback((points: number, allAnswers: Array<{ points: number; text: string }>) => {
    ...
    if (currentRevealSoundRef.current) {
      ...
      if (!isPreloadedSound(currentRevealSoundRef.current)) {
  ...
  const playWrongAnswerSound = useCallback(() => {
    ...
    if (currentWrongSoundRef.current) {
      ...
      if (!isPreloadedSound(currentWrongSoundRef.current)) {
  ...
  const playAddStrikeSound = useCallback(() => {
    ...
    if (currentStrikeSoundRef.current) {
      currentStrikeSoundRef.current.stop();
      if (!isPreloadedSound(currentStrikeSoundRef.current)) {
        currentStrikeSoundRef.current.unload();
      }
      currentStrikeSoundRef.current = null;
    }

    strikeTimeoutRef.current = setTimeout(() => {
      try {
        const customStrikeUrl = getAudioUrl('wrong_answer');
        let sound: Howl;
        ...
        if (customStrikeUrl) {
          sound = new Howl({
            src: [customStrikeUrl],
            volume: sfxVolume,
            onload: () => console.log('Custom add strike sound loaded'),
            onloaderror: (id, error) => console.error('Custom add strike sound load error:', error),
          });
        } else {
          // Fallback to wrongAnswerSound if no custom add_strike sound
          sound = wrongAnswerSound;
        }

        currentStrikeSoundRef.current = sound;
        sound.volume(sfxVolume);
        const playResult = sound.play();
        console.log('Playing add strike sound, custom:', !!customStrikeUrl, 'result:', playResult);
      } catch (error) {
  ...
  return { playRevealSound, playWrongAnswerSound, playAddStrikeSound, stopRevealSound, stopWrongAnswerSound };
};
